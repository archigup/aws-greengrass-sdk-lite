...
static GglError call_sub_callback(
    ...
    return sub_callback(
        sub_callback_ctx, service_model_type, ggl_obj_into_map(response)
    );
}
...
static GglError dispatch_incoming_packet(
...

    {
        ...
        if (!found) {
            GGL_LOGE(
                "Unhandled eventstream packet with stream id %" PRId32
                " dropped.",
                stream_id
            );
            return GGL_ERR_OK;
